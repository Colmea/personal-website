...
export default function Model(props: Props & JSX.IntrinsicElements['group'] ) {
  ...
  const [screenPosition, setScreenPosition] = useState({ x: window.innerWidth / 2, y: window.innerHeight / 2});
  ...
  // Update screen position randomly
  const speed = 0.05;
  ...
  useFrame((state, delta) => {
    if (screenDefaultRotation.current) {
      ...
      let degrees = getMouseDegrees(screenPosition.x, screenPosition.y, 15);

      const defaultZDeg = THREE.MathUtils.radToDeg(screenDefaultRotation.current.z);
      const defaultXDeg = THREE.MathUtils.radToDeg(screenDefaultRotation.current.x);
      const currentZDeg = THREE.MathUtils.radToDeg(screenRef.current.rotation.z);
      const currentXDeg = THREE.MathUtils.radToDeg(screenRef.current.rotation.x);
      
      const newXDeg = lerp(currentXDeg, defaultXDeg + (degrees.y), 0.01);
      const newZDeg = lerp(currentZDeg, defaultZDeg + (degrees.x), 0.01);
      screenRef.current.rotation.x = THREE.Math.degToRad(newXDeg);
      screenRef.current.rotation.z = THREE.Math.degToRad(newZDeg);
    }
  });
